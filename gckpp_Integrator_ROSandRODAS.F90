...
SUBROUTINE INTEGRATE( TIN,       TOUT,      ICNTRL_U, RCNTRL_U,  &
   ...
   IF ( PRESENT( RCNTRL_U ) ) THEN
      WHERE( abs(RCNTRL_U) > 0 ) RCNTRL = RCNTRL_U
   ENDIF
...
!~~~>  Parameters of the Rosenbrock method, up to 6 stages
   ...
   INTEGER, PARAMETER :: RS2=1, RS3=2, RS4=3, RD3=4, RD4=5, RG3=6, RDE=7
...
! ~~~~ Local variables
   ...
   REAL(kind=dp) :: Err, Yerr(N), ErrOld, FacOld, kk, bb
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  SUBROUTINE RodasExt(gamma, alpha_21, gamma_31, b2)
    ...
    B(2) = b2
    B(3) = (1._dp/3._dp)-gamma-(b2*A(2)**2)
    B(4) = gamma
    B(1) = 1._dp - sum(B(2:4))
    ...
    BETA_ij(4,2) = B(2)
    BETA_ij(4,3) = B(3)
    BETA_ij(2,1) = (0.5_dp-(1._dp-gamma)*B(3)-(2._dp*gamma)+gamma**2)/B(2) 
    ...
    ALPHA(2,1) = alpha_21
    ALPHA(4,3) = gamma
    ALPHA(4,2) = (onesixth-gamma+gamma**2-B(4)*(B(2)*BETA_IJ(2,1)+(1._dp-gamma)*B(3)))/(B(3)*BETA_IJ(2,1))
    ALPHA(4,1) = 1._dp - ALPHA(4,2) - ALPHA(4,3)
    ALPHA(3,1) = ALPHA(4,1) - GAM(3,1)
    ALPHA(3,2) = 1._dp - ALPHA(3,1)

    BETA_ij(3,1) = ALPHA(4,1)
    BETA_ij(3,2) = ALPHA(4,2)

    GAM(3,2) = ALPHA(4,2) - ALPHA(3,2)
    GAM(4,1) = BETA_IJ(4,1) - ALPHA(4,1)
    GAM(4,2) = BETA_IJ(4,2) - ALPHA(4,2)
    GAM(2,1) = BETA_IJ(2,1) - ALPHA(2,1)

    ! Start the operations here
    GAMinv = GAM
    call DTRTRI('L','N',4,GAMinv,4,ok)

    ! Compute M & M_
    call DGEMV('T',4,4,1._dp,GAMinv,4,B,1,0._dp,M,1)
    call DGEMV('T',4,4,1._dp,GAMinv,4,ALPHA(4,:),1,0._dp,M_,1)

    ! Compute A
    call DGEMM('N','N',4,4,4,1._dp,ALPHA,4,GAMinv,4,0._dp,A_ij,4)
...
!    write(*,*) 'ros_A(4) = ', ros_A(4)
!    write(*,*) 'ros_A(5) = ', ros_A(5)
!    write(*,*) 'ros_A(6) = ', ros_A(6)
...
!    write(*,*) 'ros_C(4) = ',ros_C(4)
!    write(*,*) 'ros_C(5) = ',ros_C(5)
